...
- Nominal Roll management lives on `/nominal-roll` in the collapsed-by-default
  "Roll management" expander in its own card directly below the roll selector,
  above the Filters card (issue 22 — it acts on the selected roll, so it sits
  next to the selector; merged from the retired
